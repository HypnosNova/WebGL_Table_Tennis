...
var readyStateCheckInterval = setInterval(function() {
    if (document.readyState === "complete") {
        var sp = document.getElementById('single');
        var mp = document.getElementById('multiplayer');
        sp.addEventListener("click", init, true);
...
function init() {
    document.body.removeChild(document.getElementById('mode'));
    ...
    var material = new THREE.MeshBasicMaterial( { color: 0xffffff, map:THREE.ImageUtils.loadTexture( "images/circle.png"), transparent: true } );
    ...
    var material = new THREE.MeshLambertMaterial( { color: 0x666666, map:THREE.ImageUtils.loadTexture( "images/ball2.jpg"), ambient: 0xcccccc, specular: 0xffffff, shininess: 30 } );
    ...
    shadowMaterial = new THREE.MeshBasicMaterial( { color: 0xffffff, map:THREE.ImageUtils.loadTexture( "images/shadow.png"), transparent: true } );
    ...
    loader.load( "models/paddle.js", paddleLoaded );
    ...
    try {
        ...
        // need webgl
        alert('Требуется поддержка WebGL.')
        return;
    }
...
function tableLoaded( geometry ) {
    ...
    var material = new THREE.MeshBasicMaterial( {opacity:0.5, transparent: true, color: 0x222222, map:THREE.ImageUtils.loadTexture( "/images/net.png")} );
    ...
    var material = new THREE.MeshPhongMaterial( {color: 0x000000, map:THREE.ImageUtils.loadTexture( "images/stiga.png"), specular: 0x545454} );
    var frontLeft = new THREE.Mesh(frontPlane, material);
    frontLeft.position.set(25,31.6,-56.4);
    frontLeft.rotation.x = -Math.PI;
    frontLeft.rotation.z = Math.PI;
    ...
    frontRight.rotation.x = -Math.PI;
    frontRight.rotation.z = Math.PI;
    ...
    netRight.rotation.x = -Math.PI;
    netRight.rotation.z = Math.PI;
    ...
    netLeft.rotation.x = -Math.PI;
    netLeft.rotation.z = Math.PI;
    netLeft.scale.set(0.25,0.25,0.25);
    netLeft.matrixAutoUpdate = false;
    netLeft.updateMatrix();
    scene.add(netLeft);

    var m = new THREE.MeshPhongMaterial( { color: 0xffffff, map:THREE.ImageUtils.loadTexture( "images/surface0.jpg"), ambient: 0x000000, specular: 0x545454 } );
    m.map.repeat.x = 0.1;
...
function paddleLoaded( geometry ) {
    ...
    var material = new THREE.MeshPhongMaterial( {color: 0xffffff, transparent: true, map:THREE.ImageUtils.loadTexture( "images/mark.png"), specular: 0x333333} );
    ...
    loader.load( "models/table.js", tableLoaded );
